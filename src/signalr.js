(function webpackUniversalModuleDefinition(root, factory) {
	...
	return /******/ (() => {
		...
		async function sendMessage(
			...
			const responseType = isArrayBuffer(content)
				? "arraybuffer"
				: "text";
		...
		class FetchHttpClient extends HttpClient {
			...
			async send(request) {
				...
				const content = deserializeContent(
					response,
				...
				for (let index = 0; index < this._messages.length; index++) {
					...
					if (element._id <= ackMessage.sequenceId) {
						...
						if (isArrayBuffer(element._message)) {
							this._bufferedByteCount -=
								element._message.byteLength;
						} else {
		...
		class HubConnection {
			...
			constructor(
				...
				this.connection.onreceive = (data) =>
			...
			async _startInternal() {
				...
				try {
					...
					await this._sendMessage(
						this._handshakeProtocol.writeHandshakeRequest(
							handshakeRequest
						)
					);
					this._logger.log(
					...
					if (useStatefulReconnect) {
						...
						this.connection.features.disconnected =
							this._messageBuffer._disconnected.bind(
								this._messageBuffer
							);
			...
			stream(methodName, ...args) {
				...
				promiseQueue = this._sendWithProtocol(
			...
			_sendWithProtocol(message) {
				if (this._messageBuffer) {
					...
					return this._sendMessage(
						this._protocol.writeMessage(message)
					);
				}
			}
				...
				const p = new Promise((resolve, reject) => {
					...
					this._callbacks[invocationDescriptor.invocationId] = (
						...
						if (error) {
							...
							// invocationEvent will not be null when an error is not passed to the callback
							if (
								invocationEvent.type === MessageType.Completion
							) {
								if (invocationEvent.error) {
			...
			_processIncomingData(data) {
				...
				if (data) {
					// Parse the messages
					const messages = this._protocol.parseMessages(
						data,
						this._logger
					);
					for (const message of messages) {
						...
						switch (message.type) {
							...
							case MessageType.Completion: {
								const callback =
									this._callbacks[message.invocationId];
								if (callback) {
									if (
										message.type === MessageType.Completion
									) {
										delete this._callbacks[
									...
									try {
										...
										this._logger.log(
											LogLevel.Error,
											`Stream callback threw error: ${getErrorString(
												e
											)}`
										);
									}
			...
			_resetTimeoutPeriod() {
				if (
					...
					if (this._pingServerHandle === undefined) {
						let nextPing =
							this._nextKeepAlive - new Date().getTime();
						if (nextPing < 0) {
							nextPing = 0;
						}
			...
			async _invokeClientMethod(invocationMessage) {
				...
				const expectsResponse = invocationMessage.invocationId
					? true
					: false;
			...
			_completeClose(error) {
				if (this._connectionStarted) {
					...
					try {
						this._closedCallbacks.forEach((c) =>
							c.apply(this, [error])
						);
					} catch (e) {
						this._logger.log(
			...
			async _reconnect(error) {
				...
				if (this._reconnectingCallbacks.length !== 0) {
					try {
						this._reconnectingCallbacks.forEach((c) =>
							c.apply(this, [error])
						);
					} catch (e) {
						this._logger.log(
				...
				while (nextRetryDelay !== null) {
					...
					await new Promise((resolve) => {
						this._reconnectDelayHandle = setTimeout(
							resolve,
							nextRetryDelay
						);
					});
						...
						complete: () => {
							promiseQueue = promiseQueue.then(() =>
								this._sendWithProtocol(
									this._createCompletionMessage(streamId)
								)
							);
						},
						error: (err) => {
			...
			_isObservable(arg) {
				...
				return (
					arg && arg.subscribe && typeof arg.subscribe === "function"
				);
			}
			...
			async connect(url, transferFormat) {
				...
				return new Promise((resolve, reject) => {
					...
					if (Platform.isNode || Platform.isReactNative) {
						...
						webSocket = new this._webSocketConstructor(
							url,
							undefined,
							{
			...
			send(data) {
				if (
					...
					this._webSocket.readyState ===
						this._webSocketConstructor.OPEN
				) {
					this._logger.log(
						LogLevel.Trace,
		...
		class HttpConnection {
			...
			async start(transferFormat) {
				...
				this._startInternalPromise =
					this._startInternal(transferFormat);
			...
			async _startInternal(transferFormat) {
				...
				try {
					if (this._options.skipNegotiation) {
						...
						do {
							negotiateResponse =
								await this._getNegotiationResponse(url);
							...
							if (negotiateResponse.accessToken) {
								...
								this._httpClient._accessTokenFactory =
									undefined;
							}
							redirects++;
						} while (
			...
			async _createTransport(
				...
				for (const endpoint of transports) {
					...
					if (transportOrError instanceof Error) {
						...
						if (!negotiate) {
							try {
								negotiate = await this._getNegotiationResponse(
									url
								);
							} catch (ex) {
			...
			_resolveTransportOrError(
				...
				if (transport === null || transport === undefined) {
					...
					if (transportMatches(requestedTransport, transport)) {
						...
						if (
							transferFormats.indexOf(requestedTransferFormat) >=
							0
						) {
							if (
		...
		class TransportSendQueue {
			...
			_bufferData(data) {
				if (
					this._buffer.length &&
					typeof this._buffer[0] !== typeof data
				) {
					throw new Error(
